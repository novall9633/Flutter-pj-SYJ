void main(List<String> args) {
  ...
  Dog d1 = Dog();
  print("나의 강아지 종류 이름은 ${d1.name}이다");
  // 클래스의 속성값 업데이트
  d1.name = '닥스훈트';
  print('1년후 나의 새로운 강아지 종류명은 ${d1.name}이다');
  // 중요한것은 클래스의 속성자체의 정의다
  // 값은 그것을 사용하는 사용자의 등록에 따라 달라질 수 있다!

  print('나의 강아지의 색깔은 ${d1.color}이고 나이는 ${d1.age}살이다');
  ...
  final bb; // final은 선언 후 처음 할당되는 것을 상수화함!
  bb='bb';
  // bb='bbb'; -> 재할당은 불가함!
  print('${aa} ${bb}');
  ...
  print('내 고양이는 ${c2.name}이다. 나이는 ${c2.age}살이다. 울음소리는 ${c2.hearSound(c2.species)}한다! 고양이의 성격은 ${c2.chracter}이다.');
...
-> 변수는 선언만 할 수도 있고 할당까지 할 수 있다
-> 생성자메서드는 클래스를 사용시 가장 먼저 호출되는 메서드
-> 개별적인 클래스 구성 메서드 생성할 수 있음

[ 다트의 클래스 상속 ]
- 부모 클래스의 모든 속성과 메서드를 자식클래스에서
사용코자 할 때 기본 공유를 하도록 하는 클래스장치
-상속 설정방법:
  class 클래스명 extends 부모 클래스명{
    구현코드...
  }
  ->만약 부모클래스 맴버필드가 초기화가 필요한 경우
  상속받은 클래스 생성시 반드시 부모 클래스 생성자를 먼저 호출하기 때문에 부모클래스의 값을 먼저 초기화 셋팅해야 한다
...
class Pet{
  ...
  // 성격
  final String chracter;
  // 먹이종류
  final String food;

  Pet(this.species,this.chracter,this.food){
...
class Cat extends Pet{
  ...
  // this키워드 : 클래스 내부 맴버(속성)을 지칭함
  // 생성자메서드(){} -> 소괄호안에 콤마로 초기값 셋팅함
  // 중괄호를 생략할 수 있다(구현코드가 없다면................)
  // -> 생성자 메서드();

  // 상속받은 부모 멤버필드 초기화는 
  // 이니셜라이즈(initialize) 키워드 콜론(:) 을 사용하여 
  // 그 뒤에 super 키워드(부모클래스)로 값을 초기화한다
