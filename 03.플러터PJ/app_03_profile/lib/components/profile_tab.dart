...
class _ProfileTabState extends State<ProfileTab>
  ...
  // 다중상속으로 "씽글티커프로바이더스테이트믹스인" 클래스 상속
  ...
  Widget build(BuildContext context) {
    // 컨텐츠 진행 방향이 수직이므로 Column() 사용
    return Column(
      children: [
        // 1. 탭바 메뉴 : _buildTabBar()
        _buildTabBar(),
        // 2. 탭 컨텐츠 영역(확장 영역 사용 - Expanded) : _buildTabBarView()
        Expanded(child: _buildTabBarView(),),
      ],
    );
  } //build ///

  // 탭 메뉴를 탭하여 컨텐츠를 변경할 것이므로
  // 탭 컨트롤러가 필요함!!!
  ...
  @override
  void initState() {
    // 1. 부모 클래스인 State()의  초기화메서드를 호출함
    super.initState();
    // 2. 탭 컨트롤로 변수에 탭컨트롤러 옵션을 설정하여 할당 함
    _tabController = TabController(length: 2, vsync: this);
    // length 속성 : 사용할 탭 개수
    // vsync 속성 : 탭 변경시 탭 메뉴와 탭뷰를 연결하는 설정
    // -> this 키워드로 현재 탭 자신을 등록함
  } //initState 재정의 ////
  ...
  // [ 2 ] 탭바 뷰 위젯 리턴 메서드 : _buildTabBarView()
  Widget _buildTabBarView() {
    // 탭바뷰 위젯 리턴 ////
    return TabBarView(
      // 반드시 컨트롤러를 연결해줘야함! 중요!!!
      controller: _tabController,
      // 탭 개수가 2개이므로 2개의 탭바뷰를 구성함
      children: [
        Container(
          color: Colors.blue,
        ),
